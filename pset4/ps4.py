...
def generate_models(x, y, degs):
    ...
    #print(type(data))
    for deg in degs:
        #print(pylab.polyfit(x,y, deg))
        data.append(pylab.polyfit(x,y, deg))
        #test = pylab.polyfit(x,y, deg)
        #print(test)
...
def r_squared(y, estimated):
    ...
    #calculating R squared based on the formula in book page 216
    estimateError = ((predicted - measured)**2).sum()
    Meanofmeasured = measured.sum()/float(len(measured))
    
    #denomenator in the formula
    variablity = ((measured - Meanofmeasured)**2).sum()
    
    #applying formula
    return 1 - estimateError/variablity
...
def evaluate_models_on_training(x, y, models):
    ...
    #for model
    mod = generate_models(x,y,models)
    #print(mod)
